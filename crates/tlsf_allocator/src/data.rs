...
impl AddrPointer {
    ...
    pub fn get_mut_block_header(&self) -> &mut BlockHeader {
        unsafe{
            return &mut (*(self.addr as *mut BlockHeader));
        }
    }
    ...
    pub fn get_mut_controller(&self) -> &mut Controller {
        unsafe{
            return &mut (*(self.addr as *mut Controller));
        }
    }
    ...
    pub fn set_prev_phy_pointer(&self, prev_phy: AddrPointer){
    ...
    pub fn set_prev_free_pointer(&self, prev_free: AddrPointer){
    ...
    pub fn set_next_free_pointer(&self, next_free: AddrPointer){
    ...
    ///Controller的功能
    pub fn init_controller(&self,addr: usize, size: usize){
        self.get_mut_controller().init(addr,size);
    }
    pub fn add_memory(&self, addr: usize, size: usize){
        self.get_mut_controller().add_memory(addr, size);
    }
    pub fn add_into_list(&self, block: AddrPointer){
        self.get_mut_controller().add_into_list(block);
    }
    pub fn del_into_list(&self, block: AddrPointer){
        self.get_mut_controller().del_into_list(block);
    }
    pub fn get_first_block(&self, fl: usize, sl: usize) -> AddrPointer{
        return self.get_mut_controller().get_first_block(fl, sl);
    }
    pub fn find_block(&self, size: usize) -> AddrPointer{
        return self.get_mut_controller().find_block(size);
    }
...
impl BlockHeader {
    ...
    pub fn set_used(&mut self){
        let next = get_block_phy_next(self.get_my_pointer());
        self.set_now_used();
        if !(next.is_null()){
            next.set_prev_used();
        }
...
pub fn get_block_phy_prev(block: AddrPointer) -> AddrPointer{
    return block.get_prev_phy_pointer();
}
...
impl Controller{
    ...
    pub fn add_memory(&mut self, addr: usize, size: usize){
        ...
        //尾部再加一个null块，占32字节
        let tail = get_addr_pointer(addr + size - 4 * size_of::<usize>());
        tail.set_null();
    }
